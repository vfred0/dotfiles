...
void drawbar(Monitor *m) {
  ...
  if (w > bh_n) {
    if (m->sel) {
      ...
      drw_setscheme(drw, scheme[SchemeNorm]);
      if(floatbar){
        drw_rect(drw, x, y, 60 - m->gappov * 2, bh_n, 1, 1);
      }else{
        drw_rect(drw, x, y, 50, bh_n, 1, 1);
      }
    }
  }
...
//     if (XGetClassHint(drw->dpy, c->win, &ch)) {
//         res = ch.res_class ? ch.res_class : NULL;
//         // XFree(ch.res_name);
//         // XFree(ch.res_class);
//     }

//     // if (!res && XFetchName(drw->dpy, c->win, &res)) {
//     //     res = strdup(res);
//     // }
...
void focus(Client *c) {
  ...
  if (c) {
    ...
    // XWarpPointer(dpy, None, c->win, 0, 0, 0, 0, attr.width / 2, attr.height / 2);
...
void warp(const Client *c) {
	...
	if((x > c->x && y > c->y && x < c->x + c->w && y < c->y + c->h) || (y > c->mon->by && y < c->mon->by + bh))
		return;
	XWarpPointer(dpy, None, c->win, 0, 0, 0, 0, c->w / 2, c->h / 2);
}
